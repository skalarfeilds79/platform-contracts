...
library StorageWrite {
    ...
    function _getArraySlot(uint _dest, uint _index) internal view returns (uint slot) {
        assembly {
            let free := mload(0x40)
            mstore(free, _dest)
    ...
    function _writeItem(uint[] memory _slots, uint _slotOffset, uint _size, uint _index, uint _value) internal {
        uint p = _index / 16;
        uint x = _index - (16 * p);
        if (_index < 16) {
            x += _slotOffset;
