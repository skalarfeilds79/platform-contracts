...
describe('Rare Pack', () => {
  ...
  describe('mint', () => {
    ...
    async function mintTrackGas(id: number, description: string) {
      const count = await rare.commitmentCount();
      const c = await rare.commitments(count.sub(1));
      const beacon = await rare.beacon();
      const contract = Beacon.at(owner, beacon);
      //await contract.randomnessOrCallback(c.commitBlock);
      // await rare.mint(count.sub(1));
      const tx = await rare.mint(count.sub(1));
