...
export class CoreStage implements DeploymentStage {
  ...
  async deploy(
    findInstance: (name: string) => Promise<string>, // TODO: [AN >> KK] This needs to be a promise to comply with parent definition of same function
    ...
    const walletImplementation =
      (await findInstance('WalletImplementation')) || (await wrapper.deployWalletImplementation()).address; // TODO: [AN >> KK] need to await findInstance

    await onDeployment('WalletImplementation', walletImplementation, false);

    console.log('** Deploying Registry **');
    const registry = await findInstance('Registry') || (await wrapper.deployRegistry(0)).address; // TODO: [AN >> KK] Need to await findInstance
    ...
    const limitedModule =
      await findInstance('LimitedModules') || (await wrapper.deployLimitedModules(registry)).address; // TODO: [AN >> KK] Need to await findInstance
    await onDeployment('LimitedModules', limitedModule, false);

    console.log('** Deploying Simple Delegate **');
    const delegate =
      await findInstance('SimpleDelegate') || (await wrapper.deploySimpleDelegate()).address; // TODO: [AN >> KK] Need to await findInstance
    await onDeployment('SimpleDelegate', delegate, false);

    console.log('** Deploying Multi Limiter **');
    const limiter = await findInstance('MultiLimiter') || (await wrapper.deployMultiLimiter()).address;  // TODO: [AN >> KK] Need to await findInstance
    await onDeployment('MultiLimiter', limiter, false);

    console.log('** Deploying Factory **');
    const factory =
      await findInstance('WalletFactory') ||  // TODO: [AN >> KK] Need to await findInstance
